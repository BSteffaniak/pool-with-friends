...
| Desktop smoke | Chrome or Chromium | Google Chrome 151.0.7922.138; macOS 26.6.1; MacBook Air (M4); 2026-08-16 | Pass — automated WebGL2/SwiftShader loads at 1280×720 reached the ready canvas on normal, default-feasibility, and reduced-feasibility entry points; required capture controls and tier reporting were present |
...
The desktop environment currently has no connected iOS or Android hardware and no Firefox/geckodriver installation, so those rows cannot be honestly completed from automation here. `./scripts/test-firefox-smoke.sh` is ready to validate normal and feasibility entry points once `FIREFOX_BIN` and `GECKODRIVER_BIN` are available. Safari 26.6 and its WebDriver are installed, but Safari rejects WebDriver sessions until the user enables **Allow remote automation** in Safari's Developer settings. After enabling it, run `./scripts/test-safari-smoke.sh` to exercise the same ready-state boundary in desktop Safari. The rows remain pending until those commands succeed; enabling a browser security setting automatically is intentionally not part of the scripts, and a driver session alone would not substitute for the required physical interaction and visual checks.
