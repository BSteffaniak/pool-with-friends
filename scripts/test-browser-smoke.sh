...
for control in test-platform hardware-model os-version browser-family browser-version cache-state minimum-version-run presentation-tier run-number physical-checks first-visible-ms first-input-ms steady-memory-mib peak-memory-mib thermal-result reload-observed capture-toggle audio-probe mark-event download-report feasibility-status; do
